...
const API = {
  async login(password, username) { return _post('/api/auth/login', { password, username }); },
  async verify() { return _get('/api/auth/verify'); },
  async changePassword(oldPwd, newPwd) { return _put('/api/auth/password', { old_password: oldPwd, new_password: newPwd }); },
  async getConfig(platform) { return _get('/api/config' + (platform ? '?platform=' + platform : '')); },
  async updateConfig(config, platform) { const d = { ...config, _platform: platform || '' }; return _put('/api/config', d); },
  async getTasks(platform) { return _get('/api/tasks' + (platform ? '?platform=' + platform : '')); },
  async getTask(id) { return _get(`/api/tasks/${id}`); },
  async initTask(platform) { return _post('/api/tasks/init', { platform: platform || 'jst' }); },
  async updateTask(id, data) { return _put(`/api/tasks/${id}`, data); },
  async deleteTask(id) { return _del(`/api/tasks/${id}`); },
  async exportTask(id) { return _get(`/api/tasks/${id}/export`); },
  async pushTask(id, testMode) { return _post(`/api/tasks/${id}/push`, testMode ? { test_mode: true } : {}); },
  async retryPlan(taskId, planId) { return _post(`/api/tasks/${taskId}/plans/${planId}/retry`, {}); },
  async getPushPlans(id) { return _get(`/api/tasks/${id}/plans`); },
  async getUsers() { return _get('/api/users'); },
  async createUser(username, password, role) { return _post('/api/users', { username, password, role }); },
  async toggleUser(id) { return _put(`/api/users/${id}/toggle`, {}); },
  async updateUserCredits(id, action, amount) { return _put(`/api/users/${id}/credits`, { action, amount }); },
  async getMyCredits() { return _get('/api/users/me/credits'); },
  async getUserWebhook(id) { return _get(`/api/users/${id}/webhook`); },
  async updateUserWebhook(id, cfg) { return _put(`/api/users/${id}/webhook`, cfg); },
